-- Add default_vacation_days column to company_settings table (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='company_settings' AND column_name='default_vacation_days'
    ) THEN
        ALTER TABLE company_settings ADD COLUMN default_vacation_days INTEGER DEFAULT 25;
    END IF;
END $$;
...
-- Add vacation tracking columns to employees table (if not exists)
DO $$ 
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='employees' AND column_name='vacation_days_total'
    ) THEN
        ALTER TABLE employees ADD COLUMN vacation_days_total INTEGER DEFAULT 25;
    END IF;
    
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_name='employees' AND column_name='vacation_days_used'
    ) THEN
        ALTER TABLE employees ADD COLUMN vacation_days_used INTEGER DEFAULT 0;
    END IF;
END $$;
...
DROP POLICY IF EXISTS "Employees can view own vacation requests" ON vacation_requests;
DROP POLICY IF EXISTS "Employees can insert own vacation requests" ON vacation_requests;
DROP POLICY IF EXISTS "Managers can view all vacation requests" ON vacation_requests;
DROP POLICY IF EXISTS "Managers can update vacation requests" ON vacation_requests;
...
-- RLS Policy: ALL authenticated users can view vacation requests (simplified for now)
-- This allows both employees and managers to see requests
CREATE POLICY "All users can view vacation requests" ON vacation_requests
    FOR SELECT USING (auth.role() = 'authenticated');

-- RLS Policy: ALL authenticated users can update vacation request status (simplified for now)
-- In a real app, you'd want to check user permissions more strictly
CREATE POLICY "All users can update vacation requests" ON vacation_requests
