    ...
    WITH matches AS (
        SELECT 
            ...
            CASE
                ...
                -- Name similarity (using built-in similarity if available)
                WHEN similarity(lower(s.name), name_normalized) > 0.8 THEN 
                    0.60 + (similarity(lower(s.name), name_normalized) * 0.3)
                -- Partial name match
                WHEN lower(s.name) LIKE '%' || name_normalized || '%' OR 
                     name_normalized LIKE '%' || lower(s.name) || '%' THEN 0.50
                ELSE 0.00
            END as score,
            CASE
                ...
                WHEN similarity(lower(s.name), name_normalized) > 0.8 THEN 'High name similarity'
                WHEN lower(s.name) LIKE '%' || name_normalized || '%' OR 
    ...
    duplicates AS (
        SELECT 
            ...
            jsonb_build_object(
                ...
                'existing_supplier', ei.supplier_name,
...
BEGIN
    WITH stats AS (
        SELECT 
            ...
            SUM(gross_total) FILTER (WHERE payment_status = 'unpaid') as unpaid_amount,
            SUM(gross_total) FILTER (WHERE payment_status = 'overdue') as overdue_amount
        FROM public.supplier_invoices
