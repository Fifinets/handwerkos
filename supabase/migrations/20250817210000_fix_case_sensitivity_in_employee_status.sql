-- Fix case sensitivity issue in employee status
-- Update the handle_new_user function to set employee status to 'Aktiv' (uppercase) instead of 'aktiv' (lowercase)

-- First, update any existing employees with lowercase 'aktiv' status to uppercase 'Aktiv'
UPDATE public.employees 
SET status = 'Aktiv' 
WHERE status = 'aktiv';

-- Update the handle_new_user function to use correct case
...
BEGIN
  ...
  IF employee_record.id IS NOT NULL THEN
    ...
    UPDATE public.employees 
    SET user_id = new.id,
        status = 'Aktiv'  -- Fixed: Use uppercase 'Aktiv' instead of lowercase 'aktiv'
