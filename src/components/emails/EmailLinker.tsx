...
/**
 * EmailLinker provides dropdowns to link an email to a specific project and/or
 * customer. On selection change it updates the `emails` table accordingly.
 */
const EmailLinker: React.FC<EmailLinkerProps> = ({ emailId }) => {
  ...
  useEffect(() => {
    // Fetch projects and customers for the current company
    const fetchOptions = async () => {
      try {
        ...
        // fetch current email link values
  ...
  const updateEmailLink = async (type: 'project' | 'customer', value: string | null) => {
    try {
      const updateData: { project_id?: string | null; customer_id?: string | null } = {};
      if (type === 'project') {
        updateData.project_id = value;
      } else {
        updateData.customer_id = value;
      }
      ...
      toast({ title: 'Verkn\u00fcpfung gespeichert', description: 'E-Mail wurde aktualisiert.' });
    } catch (err: unknown) {
      console.error(err);
